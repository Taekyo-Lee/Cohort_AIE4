...
hf_retriever = asyncio.run(run())  # hf_retriever is a Runnable so it supports async operations 
...
async def start_chat():
    ...
    lcel_rag_chain =  (  
            RunnablePassthrough.assign(context = itemgetter("query") | hf_retriever ).with_config({'run_name':'SNOWFLAKE_RETRIEVER'}) | 
            rag_prompt.with_config({'run_name':'LLAMA3_PROMPT'}) | 
            hf_llm.with_config({'run_name':'LLAMA3'})
        ).with_config({"run_name":"HFI_RAG"})
...
async def main(message: cl.Message):
    ...
    async for chunk in lcel_rag_chain.astream(
        ...
        await msg.stream_token(chunk) if chunk != '<|eot_id|>' else await msg.stream_token('')
